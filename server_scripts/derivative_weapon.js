...
EntityEvents.hurt(event => {//圣光
    ...
    if (player && player.mainHandItem.id != 'pointblank:aura_snake_strike') return
    event.entity.potionEffects.add('minecraft:instant_health', 1, 1)
})
EntityEvents.hurt(event => {//盛世
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:halcyon_k80') return
...
EntityEvents.hurt(event => {//民团
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:posse_prism') return
    event.entity.potionEffects.add('minecraft:resistance', 2400, 0)
})
EntityEvents.hurt(event => {//昏暗
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:murky_cross_star') return
...
EntityEvents.hurt(event => {//盗墓者
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:ghoul_sar') return
    player.heal(1)
})

EntityEvents.hurt(event => {//偏振
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:polarized_flea'
...
EntityEvents.hurt(event => {//地蚀
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:umbra_stifle'
...
EntityEvents.hurt(event => {//炼狱
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != "pointblank:inferno_rasword"
...
EntityEvents.hurt(event => {//骤停
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:fleeting_sandstorm'
...
EntityEvents.hurt(event => {//星爆
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:nova_guard') return
    event.entity.potionEffects.add('minecraft:instant_damage', 1, 1)
})
EntityEvents.hurt(event => {//折射
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:refraction_dila') return
    event.entity.potionEffects.add('minecraft:instant_damage', 1, 2)
})

EntityEvents.hurt(event => {//黑镜
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:rash_meteor_fist'
...
EntityEvents.hurt(event => {//辛迪加
    let player = event.getSource().getPlayer()
    if (player && player.mainHandItem.id != 'pointblank:syndicate_demeanour') return
